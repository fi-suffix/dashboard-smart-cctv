...
@section('page-title', 'Admin Accounts / Add Admin')
@section('timestamp', now()->format('M d, Y — h:i A'))

@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('dashboard.admin.store') }}" class="bg-dark-card border border-border-subtle rounded-xl p-6 space-y-4">
        @csrf
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" class="px-4 py-2 rounded-lg bg-accent-blue hover:bg-accent-blue-hover text-white text-sm font-medium transition-colors">Create Admin</button>
        </div>
    </form>
</div>
@endsection